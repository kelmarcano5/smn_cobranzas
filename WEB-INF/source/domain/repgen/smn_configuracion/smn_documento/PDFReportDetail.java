...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("doc_multiples_pagador");
			if (d != null)
			{
			name  = "b_payer_multi";
			label = "${lbl:b_payer_multi}";
		...
		d = rs.getString("doc_usa_moneda_alterna");
			if (d != null)
			{
			name  = "b_use_altern_coin";
			label = "${lbl:b_use_altern_coin}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("doc_afec_documento");
			if (d != null)
			{
			name  = "b_document_afec";
			label = "${lbl:b_document_afec}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
