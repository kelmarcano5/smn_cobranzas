package domain.repgen.smn_configuracion.smn_centro_cobranza;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("cco_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("rol_fecha_registro_desde");
			if (d != null)
			{
		...
		d = rs.getString("rol_fecha_registro_hasta");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
