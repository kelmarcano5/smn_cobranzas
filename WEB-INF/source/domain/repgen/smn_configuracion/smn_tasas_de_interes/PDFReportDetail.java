package domain.repgen.smn_configuracion.smn_tasas_de_interes;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("rol_vigencia");
			if (d != null)
			{
			name  = "b_effectivity_date";
			label = "${lbl:b_effectivity_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tai_porcentaje");
			if (d != null)
			{
			name  = "b_percentage";
			label = "${lbl:b_percentage}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tai_descripcion");
			if (d != null)
			{
			name  = "b_description";
