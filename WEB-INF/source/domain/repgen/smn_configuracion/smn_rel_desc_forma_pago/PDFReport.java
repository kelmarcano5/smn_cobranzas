package domain.repgen.smn_configuracion.smn_rel_desc_forma_pago;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_forma_pago_id_name");
			if (d != null)
			{
			name  = "b_key_form";
			label = "${lbl:b_key_form}";
		...
		d = rs.getString("smn_descuentos_retenciones_rf");
			if (d != null)
			{
			name  = "b_discounts";
			label = "${lbl:b_discounts}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rdf_fecha_registro_hasta");
			if (d != null)
			{
		...
		d = rs.getString("smn_descuentos_retenciones_rf_name");
			if (d != null)
			{
			name  = "b_discounts";
			label = "${lbl:b_discounts}";
		...
		d = rs.getString("rdf_fecha_registro_desde");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
		...
		d = rs.getString("smn_forma_pago_id");
			if (d != null)
			{
			name  = "b_key_form";
			label = "${lbl:b_key_form}";
