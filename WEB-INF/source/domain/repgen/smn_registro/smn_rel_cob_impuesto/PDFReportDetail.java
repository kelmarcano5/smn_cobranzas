package domain.repgen.smn_registro.smn_rel_cob_impuesto;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rci_fecha_registro_hasta");
			if (d != null)
			{
		...
		d = rs.getString("smn_codigos_impuestos_rf_name");
			if (d != null)
			{
			name  = "b_tax_code";
		...
		d = rs.getString("smn_codigos_impuestos_rf");
			if (d != null)
			{
			name  = "b_tax_code";
			label = "${lbl:b_tax_code}";
		...
		d = rs.getString("rci_fecha_registro_desde");
			if (d != null)
			{
			name  = "${lbl:b_store_date}";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("rci_tipo_movimiento");
			if (d != null)
			{
